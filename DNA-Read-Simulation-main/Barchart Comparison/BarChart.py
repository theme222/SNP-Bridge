...
def plot_pie_chart(final_list):  # function to plot the amount of groups total after simulation
    print(final_list)
    lenlist = len(set(final_list))
    print(final_list)
    fig, ax = plt.subplots()
    colors = plt.get_cmap("viridis")
    rescale = lambda y: (y-np.min(y))/(np.max(y)-np.min(y))
    x = 1 + np.arange(lenlist)
    y = []
    for i in range(lenlist):
        y.append(final_list.count(2 + i))
    plt.bar(x, y,width=1, color=colors(rescale(y)), linewidth=0.7)
    plt.show()
...
def main():
    ...
    for SNP_Count in range(configs["SNP Count Range"]["Start"], configs["SNP Count Range"]["End"] + 1):
        final_plot_list = []
        for i in range(200):
            DNA.current_insertid = 1
            DNA.global_snp = []
            ...
            DNA.mutate(type_a, type_b, SNP_Count)
            ...
            final_plot_list.append(len(bridge_output))

        plot_pie_chart(final_plot_list)
